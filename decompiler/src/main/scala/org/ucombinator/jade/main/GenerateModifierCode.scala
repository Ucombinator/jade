...
object GenerateModifierCode {
  case class ModifierInfo(accName: String, value: Int, keyword: Option[String], kind: String, description: String)
  ...
  def parseTables(document: Document): List[(String, List[ModifierInfo])] = {
    ...
    for ((kind, tableSummary) <- tables) yield {
      ...
      (kind, for (row <- table.select("tbody > tr").asScala.toList) yield {
        ...
        ModifierInfo(accName.text, stringToInt(value.text), keyword, kind, description.text /*TODO: support <code> */)
      })
    }
  }
    ...
    for ((kind, codeLiteral) <- lists) yield {
      ...
      (kind,
        for (mutable.Buffer(row: Element, description: Element) <- rows.toList) yield {
          ...
          val keyword = if (accName == "ACC_TRANSITIVE") { Some("transitive") } else { None }
          ModifierInfo(accName, stringToInt(value), keyword, kind, description.text)
        })
    }
  }
  ...
  def main(): Unit = {
    ...
    val modifierExtensions = modifierInfos.map(x => x.accName -> x.kind).groupBy(_._1).mapValues(_.map(_._2))
    val uniqueModifierInfos = modifierInfos.groupBy(_.accName).toList.map(_._2.head).sortBy(_.accName).sortBy(_.value)
    ...
    for (modifierInfo <- uniqueModifierInfos) {
      ...
      println(
        f"  case object ${modifierInfo.accName}\n" +
        f"    extends ModifierImpl(0x${modifierInfo.value}%04x, $keyword)\n" +
        f"    with " + modifierExtensions(modifierInfo.accName).map(_ + "Modifier").mkString(" with "))
    }
    println()
    ...
    for ((kind, modifierInfosForKind) <- tables ++ lists) {
      ...
      for (modifierInfo <- modifierInfosForKind.sortBy(_.value)) {
        println(f"    (/*0x${modifierInfo.value}%04x*/ ${modifierInfo.accName}.value, ${modifierInfo.accName}), // ${modifierInfo.description}")
      }
      println("  )")
    }
    println()
