...
import org.jgrapht.graph.{AsGraphUnion, DefaultEdge}
...
object Exceptions {
  def apply(cfg: ControlFlowGraph): Unit = {
    ...
    for (handler <- cfg.method.tryCatchBlocks.asScala) yield {
      val insns = new DepthFirstIterator[Insn, DefaultEdge](doms, Insn(cfg.method, handler.handler)).asScala.toList.sortBy(_.index)
