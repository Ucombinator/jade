...
object FlagsGen {
  ...
  def code(table: String): String = {
    ...
    builder.append("""// Do not edit this file by hand.  It is generated by `sbt`.
                     |
    ...
    for (flagsInfo <- uniqueFlagInfos) {
      ...
      val extensions = flagExtensions(flagsInfo.accName)
        .map(s => f"${s}Flag")
        .mkString(" with ")
    ...
    for ((kind, flagInfosForKind) <- flagInfoGroups) {
      ...
      for (flagInfo <- flagInfosForKind.sortBy(_.value)) {
        builder.append(
          f"    ( /*0x${flagInfo.value}%04x*/ ${flagInfo.accName}.value, ${flagInfo.accName}), // ${flagInfo.description}\n"
        )
      }
      builder.append("  )\n")
    }
